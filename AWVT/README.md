...
Cloudflare Tunnel (cloudflared), or permission for the development script to download it
...
Open Project Management and select Create a Project.

Enter a project name, such as Agora Video Call Prototype.

Select the secured authentication option that uses an App ID and App Certificate.

Open the project and copy its App ID.

Enable the primary App Certificate if necessary.

Copy the App Certificate and keep it private.
...
Do not add spaces around the values. Never commit .env or the App Certificate to GitHub.
...
Starts WalkBuddySessionServer/server.js on port 3001.

Serves WalkBuddyHelper/index.html on port 3002.

Creates a Cloudflare Quick Tunnel for the backend.

Creates another Cloudflare Quick Tunnel for the helper page.

Writes the generated URLs to AWVT/.env.local.

Starts Expo with the development client and clears the Metro cache.
...
Run npm run dev from AWVT.

Wait for the backend, helper page, Cloudflare tunnels and Expo to start.

Open the installed development build on the Android device.

Allow camera and microphone access.

Start a call to create a session and helper invitation.

Open the generated helper link on the second device.

Join the session from the helper page.

Confirm that both participants can see and hear each other.

Test the mute, camera and leave-call controls.
...
Identifies the Agora project

AGORA_APP_CERTIFICATE

WalkBuddySessionServer/.env

Signs RTC tokens and must remain private

PORT

WalkBuddySessionServer/.env

Selects the backend port; normally 3001

EXPO_PUBLIC_BACKEND_URL

Generated in AWVT/.env.local

Public Cloudflare URL for the session backend

EXPO_PUBLIC_HELPER_PAGE_URL

Generated in AWVT/.env.local

Public Cloudflare URL for the helper page
